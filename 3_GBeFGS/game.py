...
# Puzzle Assignment Series: Mechanical Match -- Segment III
...
# NEW FEATURE: The __lt__ method has been defined to overload the < operator for
# priority queue prioritization. The heuristic value is the criteria.
class Node:
    ...
    def __lt__(self, compare_node):
        return (self.heuristic < compare_node.heuristic)
...
# Here is my implementation of Greedy Best First Graph Search. The algorithm
# follows the exact specification in the text of a general graph search, with
# an added heuristic check. The heurisitic h(n) = puzzle_quota - current_score
...
# with the h(n) value as the criteria.
def GBeFGS(puzzle):
    explored = set()
    ...
    init_node.heuristic = newPuzzle.quota
    frontier.put((init_node.heuristic,init_node))
    while frontier != []:
        temp_node = frontier.get()
        leaf = temp_node[1]
        if leaf.State.score >= puzzle.quota:
            sol = Solve(leaf)
            return sol
        ...
        for action in action_set:
            ...
            child.heuristic = newPuzzle.quota-child.State.score
